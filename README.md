...
当前工作版本：**v2（静态初始化与原生物理两步整合版）**。已有历史标签保持不动，不重写同名旧标签。

[Real2Sim Prompt v2](skills/real2sim-prompt/SKILL.md) 包含两步：

1. 多视角首帧 → Pi3 联合初始化、核心完整物体与机器人建模、多视角投影/灰模/五问检查。
2. 真实视频动作恢复 → 事件邻域五问 → MuJoCo 接触执行与独立验收 → 回传 Blender 多视角对照。
